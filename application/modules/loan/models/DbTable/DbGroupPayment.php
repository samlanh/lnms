...
//     		exit();
    ...
    function updateGroupPayment($data){
    	...
    	try{
    		...
    		$loan_fun = $db_group->getReciptDetailById($id);
    		...
    		if(!empty($loan_fun)){
    			foreach ($loan_fun as $rs){
    				...
    				$old_penelize = $total_pay-($principle+$interest+$service_charge);
    				...
    				foreach ($fun as $row_fun){
    					...
    					if($row_fun["is_completed"]==1){
    						...
    						$arr_fun = array(
    								...
    								'total_payment_after'	=>	$principle+$interest+$old_penelize+$old_service,
    								'penelize'				=>	$old_penelize,
    								'service_charge'		=>	$old_service,
    						);
    						$this->_name= "ln_loanmember_funddetail";
    						$where = $db->quoteInto("id=?", $rs["lfd_id"]);
...
//     	$pay_option = $db->fetchOne($sql_payoption);
    	$sql="SELECT 
				(SELECT lcrm.`date_input` FROM `ln_client_receipt_money` AS lcrm,`ln_client_receipt_money_detail` AS lcrmd WHERE lcrm.`id`!=crm.id AND lcrmd.`crm_id`=lcrm.`id` AND lcrm.`loan_number`=(SELECT `loan_number` FROM `ln_client_receipt_money_detail` WHERE `crm_id`=crm.id LIMIT 1) ORDER BY lcrm.`date_input` DESC LIMIT 1) AS last_paydate ,
				  crm.*,
